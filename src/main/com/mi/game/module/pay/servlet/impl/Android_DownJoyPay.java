 ...
 * 调试未通过
 */
public class Android_DownJoyPay extends BasePay {

	public static final String PAY_KEY = ConfigUtil.getString("downjoy.payKey");
	public static final String APP_KEY = ConfigUtil.getString("downjoy.appKey");
	public static final String APP_ID = ConfigUtil.getString("downjoy.cpId");
	...
	public static void pay(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			Logs.pay.error("当乐平台callback数据：" + requestParams);
			Logs.pay.error("当乐平台callback签名(signature)：" + signature);
			...
			// 游戏合作商自定义参数
			String callbackInfo = requestParams.get("ext");
			// 如果是支付服务器,则分发回调请求
			if (PAYCENTER) {
				// 透传信息
			...
			String orderStatus = requestParams.get("result");
			if (!"1".equals(orderStatus)) {
				Logs.pay.error("订单：" + orderId + "支付状态不为1");
			...
			if (orderEntity.getState() == 0) {
				...
				orderEntity.setPayPlatForm(PayConstants.PLATFORM_DANGLE);
				payModule.savePayOrderEntity(orderEntity);
				// 处理充值
				payModule.payGold(orderEntity.getPlayerID(), orderEntity, null, PlatFromConstants.PLATFORM_DANGLE, null);
			}
	...
	private static String getSign(Map<String, String> requestParams) {
		...
		sb.append("order=").append(requestParams.get("order"));
		sb.append("&money=").append(requestParams.get("money"));
		sb.append("&mid=").append(requestParams.get("mid"));
		sb.append("&time=").append(requestParams.get("time"));
		sb.append("&result=").append(requestParams.get("result"));
		sb.append("&ext=").append(requestParams.get("ext"));
